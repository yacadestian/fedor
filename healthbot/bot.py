...
from .config import (
    DATA_DIR, LOGS_DIR, KNOWLEDGE_DIR, MSK_TZ,
    TELEGRAM_BOT_TOKEN, USERS_YAML_PATH,
)
...
def process_pdf(file_id: str, file_name: str, chat_id: str, owner_id: str = "") -> str:
    ...
    # Mirror extracted text to a .txt file next to the PDF + sync to Yandex Disk
    txt_mirror = local_path.with_suffix(".txt")
    txt_mirror.write_text(raw_text, encoding="utf-8")
    from .yadisk import sync_document
    sync_document(local_path, txt_mirror, remote_subdir="documents")
...
def _load_protocols() -> str:
    ...
    if _PROTOCOLS_CACHE is None:
        p = KNOWLEDGE_DIR / "protocols.yaml"
        if p.exists():
...
def process_message(message: dict) -> None:
    ...
    if message.get("photo"):
        ...
        try:
            ...
            # Mirror recognized text to a .txt file next to the photo + sync
            txt_path = photo_path.with_suffix(".txt")
            txt_path.write_text(ocr_text, encoding="utf-8")
            from .yadisk import sync_document
            sync_document(photo_path, txt_path, remote_subdir="photos")
    ...
    # Voice message → transcribe → process as text
    voice = message.get("voice") or message.get("audio")
    if voice:
        file_id = voice.get("file_id")
        log.info("Received voice from %s (%d bytes)", user["name"], voice.get("file_size", 0))
        send_typing(chat_id)
        try:
            from .voice import transcribe, stt_available
            if not stt_available():
                send_message(chat_id,
                             "🎙 Голосовые пока не настроены на сервере. "
                             "Напиши текстом или отправь PDF/фото.")
                return
            timestamp = datetime.now(MSK_TZ).strftime("%Y%m%d_%H%M%S")
            voice_path = DATA_DIR / f"{timestamp}_voice.ogg"
            download_file(file_id, voice_path)
            text_voice = transcribe(voice_path)
            send_message(chat_id, f"🎙 Распознал: <i>{text_voice[:500]}</i>")
            message["text"] = text_voice
        except Exception as exc:
            log.error("Voice processing failed: %s", exc)
            send_message(chat_id, f"Не удалось распознать голосовое: {exc}")
            return
